# modified version of ReLTER function to be used until finishing development in the original package
...
#' Default NA. 
#' @param temporalFilter temporal filter for the getObservation request. 
#' Defaults to latest observations
...
#' @import XML
#' @importFrom ReLTER get_sos_foi get_sos_procedure_info
#' @export
### function get_sos_obs
ReLTER.get_sos_obs<-function(sosURL, procedure, show_map=FALSE, temporalFilter="om%3AphenomenonTime%2Clatest"){
  requestObs <- paste0(
    sosURL,
  ...
  fois_info_service <- ReLTER::get_sos_foi(
    sosURL = sosURL,
    show_map = FALSE
  )
  ...
  # procedure info ----
  proc <- ReLTER::get_sos_procedure_info(
    sosURL = sosURL,
    procedure = procedure
  )
  # enrich results
